...
class SpeakerNet(nn.Module):
    ...
    def __init__(self, model, optimizer, trainfunc, nPerSpeaker, Syncbatch=False, **kwargs):
        ...
        self.DA_module = MMD(spk_clf_head=self.__L__, spk_backbone=self.__S__, **kwargs)
        if Syncbatch:
    ...
    def forward(self, data, label=None, domain_label=None):
        ...
        if label == None:
            ...
            [loss_c, L_MMD], [acc, L_MMD_return] = self.DA_module(outp, label, domain_label)
...
class ModelTrainer_ALDA(object):
    ...
    def __init__(self, speaker_model, optimizer, scheduler, gpu, mixedprec, tbxwriter=None, **kwargs):
        ...
        self.opt_e_c = self.__model__.module.DA_module.get_optimizer(optimizer, **kwargs)

        self.sche_e_c, self.lr_step, self.expected_step = self.__model__.module.DA_module.get_scheduler(scheduler, **kwargs)
    ...
    def train_network(self, loaderlist, verbose):
        ...
        # for data, data_label, domain_label in loader:
            ...
            if self.mixedprec:
                ...
                with autocast():
                    [loss_c, L_MMD], [acc, L_MMD_return] = self.__model__(data, label, domain_label)

                self.opt_e_c.zero_grad()
                # self.opt_d.zero_grad()
                lambd = min((self.total_step / self.warm_step)*self.lambd, self.lambd)

                self.scaler.scale(loss_c + lambd*L_MMD).backward(retain_graph=True)
            ...
            if verbose:
                ...
                sys.stdout.write("Loss %f Lr %.5f CAcc %2.3f%% MMD_Dist %2.3f - %.2f Hz "%(loss/counter, max(clr), top1/counter, top_d/counter, stepsize/telapsed))
                sys.stdout.flush()
                ...
                # self.tbxwriter.add_scalar('TrainDAS_dist', DAS_dist_mean, self.total_step)
