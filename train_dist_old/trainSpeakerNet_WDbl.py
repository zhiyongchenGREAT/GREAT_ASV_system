...
from SpeakerNet_WDbl import *
from DatasetLoader_alda import *
import shutil
...
parser.add_argument('--nClasses',       type=int,   default=6482,   help='Number of speakers in the softmax layer, only for softmax-based losses');
...
parser.add_argument('--train_list_0',     type=str,   default="/workspace/DATASET/server9_ssd/sdsv_voxceleb_da/vox2_trainlist_da.txt",     help='Train list 0')
parser.add_argument('--train_list_1',     type=str,   default="/workspace/DATASET/server9_ssd/sdsv_voxceleb_da/sdsv20_trainlist_da.txt",     help='Train list 1')
...
parser.add_argument('--test_list_sdsv',      type=str,   default="/workspace/DATASET/server9_ssd/sdsv20/sdsv20_triallist488_listonly.txt", help='Absolute path to the sdsv test set')
parser.add_argument('--enroll_list',    type=str,   default="",     help='Enroll list')

parser.add_argument('--train_path',     type=str,   default="/workspace/DATASET/server9_ssd/sdsv_voxceleb_da", help='Absolute path to the train set')
parser.add_argument('--test_path',      type=str,   default="/workspace/DATASET/server9_ssd/voxceleb", help='Absolute path to the test set')
parser.add_argument('--test_path_sdsv',      type=str,   default="/workspace/DATASET/server9_ssd/sdsv20", help='Absolute path to the test set')
...
parser.add_argument('--domain_imbalance_ratio',  type=json.loads,   default='{"0":"1", "1":"12"}')
...
parser.add_argument('--fitlog_Desc',    type=str,   default="epacal_wd_delta0.1_a21.0_nosigm");
parser.add_argument('--train_name',     type=str,   default="epacal_wd_delta0.1_a21.0_nosigm");
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    domain_imbalance_ratio_dict = {}
    for i in list(args.domain_imbalance_ratio.keys()):
        domain_imbalance_ratio_dict[int(i)] = int(args.domain_imbalance_ratio[i])

    # trainLoader_main = get_data_loader_alda(args.train_list_0, batch_size=args.batch_size//args.domain_classes, \
    #                     augment=args, musan_path, rir_path, max_frames, max_seg_per_spk, \
    #                     nDataLoaderThread, nPerSpeaker, train_path, \
    #                     sox_aug, class_strict_balance=False)

    dataloader_args = vars(args).copy()
    ...
    dataloader_args = vars(args).copy()
    dataloader_args.pop('batch_size')
    dataloader_args.pop('max_seg_per_spk')
    trainLoader_do1 = get_data_loader_alda(args.train_list_1, batch_size=(args.batch_size//args.domain_classes),\
                        max_seg_per_spk=int(args.max_seg_per_spk*domain_imbalance_ratio_dict[1]*1.2),\
                        class_strict_balance=True, **dataloader_args)
    ...
    while(1):
        ...
        loss, traineer, stop = trainer.train_network([trainLoader_main, trainLoader_do1], verbose=(args.gpu==0))
        ...
        if it % args.test_interval == 0 or stop == True:
            ...
            print("IT %d, GPU %d, LR %f, CAcc %2.2f, L_grad %2.2f, L_wd %2.2f, TLOSS %f, VEER %2.4f, MINEER %2.4f"%\
                (it, args.gpu, max(clr), traineer[0], traineer[1], traineer[2], loss, result[1], min(min_eer)))
            scorefile.write("IT %d, GPU %d, LR %f, CAcc %2.2f, L_grad %2.2f, L_wd %2.2f, TLOSS %f, VEER %2.4f, MINEER %2.4f\n"%\
                (it, args.gpu, max(clr), traineer[0], traineer[1], traineer[2], loss, result[1], min(min_eer)))
            scorefile.flush()
            ...
            print("IT %d, GPU %d, LR %f, CAcc %2.2f, L_grad %2.2f, L_wd %2.2f, TLOSS %f"%(it, args.gpu, max(clr), traineer[0], traineer[1], traineer[2], loss))
            scorefile.write("IT %d, GPU %d, LR %f, CAcc %2.2f, L_grad %2.2f, L_wd %2.2f, TLOSS %f\n"%(it, args.gpu, max(clr), traineer[0], traineer[1], traineer[2], loss))
            scorefile.flush()
