...
class SpeakerNet(nn.Module):
    ...
    def __init__(self, model, optimizer, trainfunc, nPerSpeaker, expected_step, Syncbatch=False, **kwargs):
        ...
        nOut_ori = loss_args.pop('nOut')

        self.__L__ = LossFunction(nOut=2*nOut_ori, **loss_args)
        if Syncbatch:
            raise NotImplementedError
    ...
    def forward(self, data, label=None, total_step=None):
        ...
        outp_S    = self.__S__.forward(data)
        outp_Ss  = self.__Ss__.forward(data)
        outp = torch.cat([outp_S, outp_Ss], dim=1)
