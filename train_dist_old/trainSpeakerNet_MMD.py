...
parser.add_argument('--fitlog_Desc',    type=str,   default="xvector_MMDsimp_1.0");
parser.add_argument('--train_name',     type=str,   default="xvector_MMDsimp_1.0");
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    while(1):
        ...
        if it % args.test_interval == 0 or stop == True:
            ...
            print("IT %d, GPU %d, LR %f, CAcc %2.2f, MMD_Dist %2.2f, TLOSS %f, VEER %2.4f, MINEER %2.4f"%\
                (it, args.gpu, max(clr), traineer[0], traineer[1], loss, result[1], min(min_eer)))
            scorefile.write("IT %d, GPU %d, LR %f, CAcc %2.2f, MMD_Dist %2.2f, TLOSS %f, VEER %2.4f, MINEER %2.4f\n"%\
                (it, args.gpu, max(clr), traineer[0], traineer[1], loss, result[1], min(min_eer)))
            scorefile.flush()
            ...
            print("IT %d, GPU %d, LR %f, CAcc %2.2f, MMD_Dist %2.2f, TLOSS %f"%(it, args.gpu, max(clr), traineer[0], traineer[1], loss))
            scorefile.write("IT %d, GPU %d, LR %f, CAcc %2.2f, MMD_Dist %2.2f, TLOSS %f\n"%(it, args.gpu, max(clr), traineer[0], traineer[1], loss))
            scorefile.flush()
