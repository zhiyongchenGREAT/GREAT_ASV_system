...
from SpeakerNet_HPM import *
from DatasetLoader_HPM import get_data_loader
import shutil
...
parser.add_argument('--spec_aug',       type=bool,  default=False,    help='Use spec aug or not');
...
parser.add_argument('--fitlog_Desc',    type=str,   default="test");
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    while(1):
        ...
        ## get ss_matrix
        current_W = s.module.__L__.W.detach().cpu() #[out, classes]
        w_norm = torch.norm(current_W, p=2, dim=0, keepdim=True).clamp(min=1e-12)
        w_norm = torch.div(current_W, w_norm)
        ss_matrix = torch.mm(w_norm.t(), w_norm)
        ## Initialise data loader base on ss matrix every epoch
        trainLoader = get_data_loader(args.train_list, ss_matrix=ss_matrix, **vars(args))    
