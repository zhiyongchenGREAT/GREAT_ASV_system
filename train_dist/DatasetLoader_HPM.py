...
class voxceleb_sampler(torch.utils.data.Sampler):
    ...
    def __init__(self, data_source, nPerSpeaker, max_seg_per_spk, batch_size, ss_matrix, \
        ...
        ## use for HPM training
        self.ss_matrix = ss_matrix # detach and cpu tensor

        self.S = 16
        self.I = 8
        self.U = 1
        assert (self.S * self.I * self.U) == self.batch_size
    ...
    def __iter__(self):
        ...
        if not self.class_strict_balance:
            for _, key in enumerate(dictkeys):
                ...
                perspk_flattened_list = []
                for indices in rp:
                    perspk_flattened_list.append([data[i] for i in indices])
                HPM_datalist_dict[key] = perspk_flattened_list
        else:
            for _, key in enumerate(dictkeys):
                ...
                perspk_flattened_list = []
                for indices in rp:
                    perspk_flattened_list.append([patched_data[i] for i in indices])
                HPM_datalist_dict[key] = perspk_flattened_list
        
            
        data_length = len(flattened_label)
        self.expect_return_length = data_length - data_length % self.batch_size
        batches = self.expect_return_length // self.batch_size
        sample_spks = batches * self.S
        ...
        ## Prevent two pairs of the same speaker in the same batch during sampling S
        for ii in mixid:
            startbatch = len(mixlabel) - len(mixlabel) % self.S
            if flattened_label[ii] not in mixlabel[startbatch:]:
                mixlabel.append(flattened_label[ii])
                # mixmap.append(ii)
        
        assert len(mixlabel) >= sample_spks
        mixlabel = mixlabel[:sample_spks]
        # mixmap = mixmap[:sample_spks]

        ## present HPM sampling
        for i in range(batches):
            S = mixlabel[i*self.S:(i+1)*self.S]
            for j in S:
                nearest_skp_list = self.ss_matrix[j].topk(self.I, 0)[1]
                for k in nearest_skp_list:
                    perspk_flattened_list = HPM_datalist_dict[int(k)]
                    HPM_flattened_list.extend(numpy.random.permutation(perspk_flattened_list)[:self.U])

        assert len(HPM_flattened_list) == self.expect_return_length
                
        ## const batch is not necessary
        if self.const_batch:
            pass
        
        return iter(HPM_flattened_list)
    
    def __len__(self):
        return self.expect_return_length


def get_data_loader(dataset_file_name, batch_size, augment, musan_path, rir_path, max_frames, max_seg_per_spk, nDataLoaderThread, nPerSpeaker, train_path, sox_aug, ss_matrix, **kwargs):
    
    train_dataset = voxceleb_loader(dataset_file_name, augment, musan_path, rir_path, max_frames, train_path, sox_aug)

    train_sampler = voxceleb_sampler(train_dataset, nPerSpeaker, max_seg_per_spk, batch_size, ss_matrix)
