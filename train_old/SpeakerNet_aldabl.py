...
class SpeakerNet(nn.Module):
    ...
    def __init__(self, model, optimizer, trainfunc, nPerSpeaker, Syncbatch=False, **kwargs):
        ...
        self.DA_module = FOCAL_ALDA_MULDO_OPT_FAST(spk_clf_head=self.__L__, spk_backbone=self.__S__, **kwargs)
        if Syncbatch:
    ...
    def train_network(self, loaderlist, verbose):
        ...
        # for data, data_label, domain_label in loader:
            ...
            if verbose:
                ...
                sys.stdout.write("Loss %f Lr %.5f CAcc %2.3f%% DAcc %2.3f%% - %.2f Hz "%(loss/counter, max(clr), top1/counter, top_d/counter, stepsize/telapsed))
                sys.stdout.flush()
