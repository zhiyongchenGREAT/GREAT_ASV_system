...
class ModelTrainer(object):
    ...
    def __init__(self, speaker_model, optimizer, scheduler, gpu, mixedprec, tbxwriter=None, **kwargs):
        ...
        # print('####################')
        # print(list(self.__model__.parameters()))
        # print(self.__model__.module.__S__.layer1.bn.bias.size())
        # print(self.__model__.module.__S__.layer1.bn.weight.size())

        bn_list = []
        selected_param = []
        ori_params_dict = {}
        for i in list(self.__model__.named_parameters()):
            ori_params_dict[i[0]] = i[1]

        # for i, param in zip(self.__model__.state_dict().keys(), self.__model__.parameters()):
        #     if 'bn' in i:
        #         if ('layer1' in i) or ('layer2' in i):
        #             # if i.split('.')[-1] == 'weight' or i.split('.')[-1] == 'bias':
        #             bn_list.append(i)
        #             # print(self.__model__.state_dict()[i].size())
        #             # print(param.size())
        #             selected_param.append(ori_params_dict[i])

        for i in ori_params_dict:
            if 'bn' in i:
                if ('layer1' in i) or ('layer2' in i):
                    # if i.split('.')[-1] == 'weight' or i.split('.')[-1] == 'bias':
                    bn_list.append(i)
        ...
        print([i[0] for i in self.__model__.named_parameters()])

        # self.__optimizer__ = Optimizer(self.__model__.parameters(), **kwargs)
        # print(list(selected_param))
        self.__optimizer__ = Optimizer(selected_param, **kwargs)
