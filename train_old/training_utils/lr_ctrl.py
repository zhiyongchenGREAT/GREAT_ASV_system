...
def sdsvc_lr_decay_ctrl(opt, total_step, optimizer, scheduler, train_log):   
    current_lr = optimizer.param_groups[0]['lr']
    scheduler_step = scheduler.state_dict()['_step_count']
    cur_anchor_log = os.path.join(opt.lr_ctrl_path, str(scheduler_step)+".log")
    metric = opt.lr_ctrl["metric"]
...
def vox1test_lr_decay_ctrl_AL(opt, total_step, optimizer, scheduler, train_log):   
    current_lr = optimizer.param_groups[0]['lr']
...
def sdsvc_lr_decay_ctrl_AL(opt, total_step, optimizer, scheduler, train_log):   
    current_lr = optimizer.param_groups[0]['lr']
    scheduler_step = scheduler[0].state_dict()['_step_count']
    cur_anchor_log = os.path.join(opt.lr_ctrl_path, str(scheduler_step)+".log")
    metric = opt.lr_ctrl["metric"]
