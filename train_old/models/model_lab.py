...
class ORI_AL(nn.Module):
    ...
    def forward(self, x, y, mod):
        ...
        loss_al = self.negloss(weight * ((1-self.sft(logits_d_al)) ** self.model_settings['focal_d_gamma']) * torch.log(self.sft(logits_d_al)), 1-y_d)
...
class DANN_tester_AL_FOCAL_REBL_full(nn.Module):
    def __init__(self, model_settings):
        super(DANN_tester_AL_FOCAL_REBL_full, self).__init__()
        self.th_step = model_settings['anneal_steps']
        self.iter = 0.0
        self.max_m = model_settings['m']
        self.model_settings = model_settings
...
class DANN_tester_AL_startpoint(nn.Module):
    def __init__(self, model_settings):
        super(DANN_tester_AL_startpoint, self).__init__()
        self.th_step = model_settings['anneal_steps']
        self.iter = 0.0
        self.max_m = model_settings['m']
        self.model_settings = model_settings
    ...
    def forward(self, x, y, mod):
        ...
        loss_al = self.loss(logits_d_al, 1-y_d)
