...
## Training details
parser.add_argument('--test_interval',  type=int,   default=200,     help='Test and save every [test_interval] epochs')
...
parser.add_argument('--nClasses',       type=int,   default=1417,   help='Number of speakers in the softmax layer, only for softmax-based losses')
...
parser.add_argument('--train_list',     type=str,   default="/workspace/DATASET/server9_ssd/sdsv21/sdsv21_normal_FA1417.txt",     help='Train list')
...
parser.add_argument('--fitlog_Desc',    type=str,   default="ECAPA-1024+Sidetune(sdsv/mozila)")
parser.add_argument('--train_name',     type=str,   default="ECAPA-1024+Sidetune(sdsv/mozila)")
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if len(modelfiles) >= 1 and not args.eval:
        ...
        ## use eval models
        trainer.loadParameters_sidetune(args.initial_model, args.initial_model, only_para=True);
        ## use old load models
