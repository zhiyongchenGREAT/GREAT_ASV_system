...
class SpeakerNet(nn.Module):
    ...
    def __init__(self, model, optimizer, trainfunc, nPerSpeaker, Syncbatch=False, **kwargs):
        ...
        self.DA_module = DAS_BL(spk_clf_head=self.__L__, spk_backbone=self.__S__, **kwargs)
        if Syncbatch:
    ...
    def forward(self, data, label=None, domain_label=None):
        ...
        if label == None:
            ...
            [loss_c, DAS_loss], [acc, DAS_dist_mean] = self.DA_module(outp, label, domain_label)
    ...
    def train_network(self, loaderlist, verbose):
        ...
        # for data, data_label, domain_label in loader:
            ...
            if self.mixedprec:
                ...
                with autocast():
                    [loss_c, DAS_loss], [acc, DAS_dist_mean] = self.__model__(data, label, domain_label)

                self.opt_e_c.zero_grad()
                # self.opt_d.zero_grad()
                lambd = min((self.total_step / self.warm_step)*self.lambd, self.lambd)

                self.scaler.scale(loss_c + lambd*DAS_loss).backward(retain_graph=True)
            ...
            if verbose:
                ...
                sys.stdout.write("Loss %f Lr %.5f CAcc %2.3f%% Dist %2.3f%% - %.2f Hz "%(loss/counter, max(clr), top1/counter, top_d/counter, stepsize/telapsed))
                sys.stdout.flush()
