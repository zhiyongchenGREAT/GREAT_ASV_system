...
from read_data import CSVDataSet, Dataloader_ShuffleWrapper, Dataset_hdf5, dataset_h5_tmp, WithReplacementRandomSampler
from torch.optim.lr_scheduler import StepLR, MultiStepLR
from config.config import Config
# import train_model_bank
...
if __name__ == '__main__':
    ...
    model = get_model(opt.model, opt.metric, opt.num_classes, opt.model_settings, opt)
    ...
    train_data = CSVDataSet(opt.train_list)
    train_dataloader = DataLoader(dataset=train_data, batch_size = opt.train_batch_size, sampler=WithReplacementRandomSampler(train_data), shuffle = False, num_workers = opt.num_workers, pin_memory=True)

    val_data = CSVDataSet(opt.val_list)
    val_dataloader = DataLoader(dataset=val_data, batch_size = 1, shuffle = False, num_workers = opt.num_workers, pin_memory=True)
    ...
    # main logger
    logger = making_log(opt.log_path)
    ...
    if opt.model_load_path != '':
        ...
        sch_statdict['last_epoch'] = checkpoint['scheduler']['last_epoch']
    ...
    for epoch in range(start_epoch, opt.max_epoch):
        ...
        for count, (batch_x, batch_y) in enumerate(train_dataloader):
            ...
            if ((count+1) % opt.print_freq) == 0:
                ...
                logger.info('Epoch:'+str(epoch+1)+' Step:'+str(count+1)+' Loss:'+str(train_loss)+' Inter:'+str(inter)+' Acc:'+str(train_acc)+' lr:'+str(current_lr))

                # draw hist plot for intra cos and weighting curve
                bins = np.linspace(-0.5, 1, 75)
                fig = plt.figure()
                plt.hist(model.cos_bank, bins, density=True, facecolor='r')
                if model.u_r != None:
                    cos_theta = np.arange(-0.5, 1.0, 0.01)
                    w_3 = np.e**((-(cos_theta-model.u_r)**2)/(2*model.sig**2))
                    plt.plot(cos_theta, w_3, 'g--')

                writer.add_figure('intra_cos', fig, total_step)
                writer_aux.add_figure('intra_cos', fig, total_step)
                fig.savefig(os.path.join(opt.train_emb_plot_path, 'e'+str(epoch+1)+'s'+str(count+1)+'.png'), dpi=100)
            ...
            if opt.val_interval_step is not None and ((count+1) % opt.val_interval_step) == 0:
                ...
                if opt.plot_emb:
                    fig = utils.make_train_emb_plot(model, opt, device)
                    fig.savefig(os.path.join(opt.train_emb_plot_path, 'e'+str(epoch+1)+'s'+str(count+1)+'.png'), dpi=100)
                    writer.add_figure('train_emb_plot', fig, total_step)
                    writer_aux.add_figure('train_emb_plot', fig, total_step)

                    fig = utils.make_test_emb_plot(model, opt, device)
                    fig.savefig(os.path.join(opt.test_emb_plot_path, 'e'+str(epoch+1)+'s'+str(count+1)+'.png'), dpi=100)
                    writer.add_figure('test_emb_plot', fig, total_step)
                    writer_aux.add_figure('test_emb_plot', fig, total_step)

                if opt.calculate_fisher:
                    Sw, Sb = utils.calculate_angular_fisher(model, opt, device)
                    writer.add_scalar('val/Sw', Sw, total_step)
                    writer.add_scalar('val/Sb_a2_clip', Sb, total_step)
                    writer_aux.add_scalar('val/Sw', Sw, total_step)
                    writer_aux.add_scalar('val/Sb_a2_clip', Sb, total_step)
                else:
                    Sw, Sb = None, None
                ...
                if (epoch+1) in opt.fine_collecting:
                    ...
                    fig, eer = utils.trial_final(model, opt, device, temporal_results_path)
                ...
                logger.info('----------')
                logger.info('ValLoss:'+str(val_loss)+' ValAcc:'+str(val_acc)+' EER:'+str(eer)+' Sw:'+str(Sw)+' Sb:'+str(Sb))
                with open(val_log_path, 'a') as f:
                    f.write('ValLoss:'+str(val_loss)+' ValAcc:'+str(val_acc)+' EER:'+str(eer)+' Sw:'+str(Sw)+' Sb:'+str(Sb)+'\n')                    
                val_loss = 0
                val_acc = 0  
                torch.backends.cudnn.benchmark = opt.cudnn_benchmark
                model.train()
        ...
        if opt.plot_emb:
            fig = utils.make_train_emb_plot(model, opt, device)
            fig.savefig(os.path.join(opt.train_emb_plot_path, 'e'+str(epoch+1)+'.png'), dpi=100)
            writer.add_figure('train_emb_plot', fig, total_step)
            writer_aux.add_figure('train_emb_plot', fig, total_step)

            fig = utils.make_test_emb_plot(model, opt, device)
            fig.savefig(os.path.join(opt.test_emb_plot_path, 'e'+str(epoch+1)+'.png'), dpi=100)
            writer.add_figure('test_emb_plot', fig, total_step)
            writer_aux.add_figure('test_emb_plot', fig, total_step)
        ...
        logger.info('----------')
        logger.info('End Epoch:'+str(epoch+1)+' ValLoss:'+str(val_loss)+' ValAcc:'+str(val_acc)+' EER:'+str(eer)+' Sw:'+str(Sw)+' Sb:'+str(Sb))
        with open(val_log_path, 'a') as f:
            f.write('End Epoch:'+str(epoch+1)+' ValLoss:'+str(val_loss)+' ValAcc:'+str(val_acc)+' EER:'+str(eer)+' Sw:'+str(Sw)+' Sb:'+str(Sb)+'\n')    
