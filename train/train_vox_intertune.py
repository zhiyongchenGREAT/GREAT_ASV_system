...
from read_data import CSVDataSet, WithReplacementRandomSampler
...
if __name__ == '__main__':
    ...
    for name, param in model.named_parameters():
        if name == 'metrics.weight':
            param.requires_grad = False
            print('last layer fixed!')
    ...
    for epoch in range(start_epoch, opt.max_epoch):
        ...
        # ############
        # temporal_results_path = os.path.join(opt.temporal_results_path, 'e'+str(epoch+1)+'s'+str(0))
        # if not os.path.isdir(temporal_results_path):
        #     os.makedirs(temporal_results_path)
        # eer = utils.trial_final(model, opt, device, temporal_results_path)
        # print(eer)
        # model.train()
        # ############
