...
def vox1test_lr_decay_ctrl(opt, total_step, optimizer, scheduler, train_log):   
    ...
    if os.path.isfile(cur_anchor_log):
        ...
        if Anchor is None or Count is None:
            raise NotImplementedError

        if metric_value+sig_th >= Anchor:
            msg = "Step: {:} S_Step: {:} Lr: {:.5f} Anchor: {:.4f} Count: {:}".format(total_step, scheduler_step, current_lr, Anchor, Count+1)
    ...
    msg = "Step: {:} S_Step: {:} Lr: {:.5f} Anchor: {:.4f} Count: {:}".format(total_step, scheduler_step, current_lr, metric_value, 0)
    with open(cur_anchor_log, 'w') as f:
        f.writelines([msg+'\n']) 
