...
def vox1test_cls_eval_AL(model, opt, total_step, optimizer, train_log, tbx_writer):
    if not hasattr(opt, 'vox_val_list'):
        return
...
def vox1test_ASV_eval(model, device, opt, total_step, optimizer, train_log, tbx_writer):
    ...
    if hasattr(opt, 'vox1test_aux_list') and hasattr(opt, 'vox1test_aux_keys'):
        score.calibrating(os.path.join(out_dir, 'calib.pth'), 50, opt.vox1test_aux_keys, [os.path.join(out_dir, 'scores_aux')])
        score.applying(os.path.join(out_dir, 'calib.pth'), [os.path.join(out_dir, 'scores')], os.path.join(out_dir, 'scores_calib'))
        eer, minc, actc = score.scoring(os.path.join(out_dir, 'scores_calib'), opt.vox1test_trial_keys, opt.scoring_config)
    else:
