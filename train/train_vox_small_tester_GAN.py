...
if __name__ == '__main__':
    ...
    model = GAN_tester(opt.model_settings)
    ...
    train_data_s = PickleDataSet_single('/Lun0/zhiyong/vox_small_evad_sparse/vox_small_evad_sparse.csv')
    train_dataloader_s = My_DataLoader(train_data_s, batch_size=opt.train_batch_size, shuffle=False, sampler=RandomSampler(train_data_s, replacement=True),\
    batch_sampler=None, num_workers=opt.num_workers, collate_fn=None,\
    pin_memory=False, drop_last=False, timeout=0,\
    worker_init_fn=None, multiprocessing_context=None)

    train_data_t = PickleDataSet_single('/Lun0/zhiyong/sdsvc_t2_small_sparse/train_data_small_sparse.csv')
    train_dataloader_t = My_DataLoader(train_data_t, batch_size=opt.train_batch_size, shuffle=False, sampler=RandomSampler(train_data_t, replacement=True, num_samples=len(train_data_s)),\
    batch_sampler=None, num_workers=opt.num_workers, collate_fn=None,\
    pin_memory=False, drop_last=False, timeout=0,\
    worker_init_fn=None, multiprocessing_context=None)
    ...
    start_epoch = 0
    min_eer = 1.0
    expected_total_step = len(train_data_s) * opt.max_epoch // opt.train_batch_size
    expected_total_step_epoch = len(train_data_s) // opt.train_batch_size
    ...
    for epoch in range(start_epoch, opt.max_epoch):
        ...
        for count, ([batch_x_s, batch_y_s], [batch_x_t, batch_y_t]) in enumerate(zip(train_dataloader_s, train_dataloader_t)):

            batch_x_s = batch_x_s.cuda(non_blocking=True)
            batch_y_s = batch_y_s.cuda(non_blocking=True)
            batch_x_t = batch_x_t.cuda(non_blocking=True)
            batch_y_t = batch_y_t.cuda(non_blocking=True)
            ...
            [loss_c, loss_d, loss_al], predict, emb, acc, acc_d = model(batch_x[:180], batch_y[:180], mod='train')
            ...
            # (loss_c + loss_al).backward()
            # for i in range(3):
            #     opt_e.step()

            # beta = min(total_step / 5000, 1.0)
            # gamma = min(total_step / 5000, 1.0)
