r"""This is a modified version of pytorch dataloader to release the limit of sequential reading.
...
class My_DataLoader(object):
    ...
    def __setattr__(self, attr, val):
        ...
        super(My_DataLoader, self).__setattr__(attr, val)
...
class _BaseDataLoaderIter(object):
    def __init__(self, loader):
        ...
        self._num_yielded = 0
        self._expected_len = len(self._index_sampler)
...
class _MultiProcessingDataLoaderIter(_BaseDataLoaderIter):
    ...
    def _next_data(self):
        while True:
            # If the worker responsible for `self._task_info` has already ended
            # and was unable to fulfill this task (due to exhausting an `IterableDataset`),
            # we try to remove them from self._task_info.
            #
            # This part needs to run (but not in the loop) because both the `self._get_data()`
            # call and `_IterableDatasetStopIteration` check below can mark
            # extra worker(s) as dead.
            if self._dataset_kind == _DatasetKind.Map and self._num_yielded < self._expected_len:
                assert len(self._task_info) > 0

            elif self._dataset_kind == _DatasetKind.Iterable and len(self._task_info) > 0:
                outstanding_worker_ids = [(idx, self._task_info[idx][0]) for idx in self._task_info if len(self._task_info[idx]) == 1]
                for idx, worker_id in outstanding_worker_ids:
                    if not self._workers_status[worker_id]:
                        del self._task_info[idx]
                assert len(self._task_info) > 0                

            else:
                # no valid task in self._task_info is found or we have produce expected num of egs
                self._shutdown_workers()
                raise StopIteration

            # Now we get the first batch and its index we fetch in the queue
            ...
            if self._dataset_kind == _DatasetKind.Iterable:
                ...
                if isinstance(data, _utils.worker._IterableDatasetStopIteration):
                    ...
                    # No need to put index here
                    continue
            
            # The new index is add to the data releasing worker.
            # It is expected that all working workers have the same num of tasks in their index queue (and should be 2).
    ...
    def _try_put_index(self, expected_worker_id=None):
        ...
        if expected_worker_id is not None:
            worker_queue_idx = expected_worker_id
