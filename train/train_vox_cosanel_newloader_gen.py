...
from background_gen import SpeakerDataGen
from read_data import CSVDataSet, WithReplacementRandomSampler, PickleDataSet
...
if __name__ == '__main__':
    ...
    start_epoch = 0
    min_eer = 1.0
    expected_total_step = len(train_data) * opt.max_epoch
    expected_total_step_epoch = len(train_data)
    logger.info('Total_step: '+str(expected_total_step))

    # init background speaker data generater
    bg_gen = SpeakerDataGen()
    ...
    if opt.model_load_path != '':
        ...
        scheduler.step()
        # swap into new data
        bg_gen.swap_and_clear_data()     

    timing_point = time.time()

    for epoch in range(start_epoch, opt.max_epoch):

        # start background speaker data generater processes
        bg_gen.setup_and_spawn_process()
        ...
        # stop background speaker generater
        bg_gen.stop_and_join_process()
        bg_gen.swap_and_clear_data()
