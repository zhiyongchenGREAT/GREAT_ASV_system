...
from read_data import CSVDataSet, WithReplacementRandomSampler, SRE_CSVDataSet_Tmp
...
if __name__ == '__main__':
    ...
    train_data = SRE_CSVDataSet_Tmp(opt.train_list)
    ...
    start_epoch = 0
    min_eer = 1.0
    expected_total_step = len(train_data) * opt.max_epoch * (1.0 / opt.train_batch_size)
    expected_total_step_epoch = len(train_data) * (1.0 / opt.train_batch_size)
    ...
    for epoch in range(start_epoch, opt.max_epoch):
        ...
        for count, (batch_x, batch_y) in enumerate(train_dataloader):
            ...
            # cut to 200-300
            # length = torch.randint(200, 300, [])
            # start = torch.randint(0, 300-length, [])
            ...
            if opt.val_interval_step is not None and ((count+1) % opt.val_interval_step) == 0:
                ...
                logger.info('----------')
                logger.info('ValLoss:'+str(val_loss)+' ValAcc:'+str(val_acc)+' EER:'+str(eer))
                with open(val_log_path, 'a') as f:
                    f.write('ValLoss:'+str(val_loss)+' ValAcc:'+str(val_acc)+' EER:'+str(eer)+'\n')                    
                val_loss = 0
                val_acc = 0  
                torch.backends.cudnn.benchmark = opt.cudnn_benchmark
                model.train()
        ...
        logger.info('----------')
        logger.info('End Epoch:'+str(epoch+1)+' ValLoss:'+str(val_loss)+' ValAcc:'+str(val_acc)+' EER:'+str(eer))
        with open(val_log_path, 'a') as f:
            f.write('End Epoch:'+str(epoch+1)+' ValLoss:'+str(val_loss)+' ValAcc:'+str(val_acc)+' EER:'+str(eer)+'\n')
