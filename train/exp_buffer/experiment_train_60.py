...
def main():
    ...
    model = FOCAL_ALDA.FOCAL_ALDA_2DO_OPT(opt.model_settings)
    ...
    _ = torch.optim.SGD(model.parameters(), lr=opt.lr, momentum=opt.momentum, weight_decay=opt.weight_decay)
    ...
    while True:
        ...
        opt_c.zero_grad()
        opt_d.zero_grad()
        beta = min((total_step / 50000)*0.05, 0.05)*beta_scale
        ...
        if (total_step % opt.val_interval_step) == 0:
            ...
            training_utils.epoch_lr_decay_ctrl_AL(expected_total_step_epoch, opt, total_step, opt_e, \
            [scheduler_e, scheduler_c, scheduler_d], train_log)
