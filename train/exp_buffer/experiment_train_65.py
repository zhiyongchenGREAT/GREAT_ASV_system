...
def main():
    ...
    model = model_lab.AM_normfree_softmax_anneal_ce_head_reweight(opt.model_settings)
    ...
    model, _, _, total_step = training_utils.resume_training(opt, model, optimizer, scheduler)
    ...
    while True:
        ...
        loss_c, _, _, acc_c, _ = model(batch_x, batch_y, mod='train')
        ...
        if (total_step % opt.print_freq) == 0:
            ...
            training_utils.standard_freq_logging(delta_time, total_step, train_loss_c, train_acc_c, optimizer, train_log, tbx_writer)
        ...
        if (total_step % opt.val_interval_step) == 0:
            ...
            training_utils.sdsvc_metric_saver(model, opt, total_step, optimizer, scheduler, train_log)
