...
def JointBayesian_Train(trainingset, label, fold = "./", complete_stats=False):
    if fold[-1] != '/':
        fold += '/'
    ...
    for l in range(20):
        ...
        if complete_stats:
            covariance = Sw - Sw.dot(F + G).dot(Sw)
            Su = np.cov(u.T,  rowvar=0) + covariance
            Sw = np.cov(ep.T, rowvar=0) + covariance
        else:
            Su = np.cov(u.T,  rowvar=0)
            Sw = np.cov(ep.T, rowvar=0)
...
def PCA_Train(data, result_fold, n_components=2000):
    print("PCA training (n_components=%d)..."%n_components)
