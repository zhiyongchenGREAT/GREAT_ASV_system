...
class PldaEstimation(object):
    ...
    """
    def __init__(self, Pldastats, use_withinvar_prior=True):
        ...
        self.nllr_m = 0.0
        self.nllr_m_2 = 0.0
        self.use_withinvar_prior = use_withinvar_prior
    ...
    def compute_object_function_part1(self):
        ...
        inv_within_var = scipy.linalg.pinvh(inv_within_var)       
    ...
    def compute_object_function_part2(self):
        ...
        for i in range(np.array(self.stats.classinfo).shape[0]):
            ...
            if n != info.num_example:
                ...
                combined_inv_var = scipy.linalg.pinvh(combined_inv_var)
    ...
    def get_stats_from_intraclass(self):
        if self.use_withinvar_prior:
            self.within_var_stats += self.stats.offset_scatter
            # self.within_var_count += self.stats.num_example - self.stats.num_classes
            self.within_var_count += self.stats.num_example - self.dim - 1
        else:
            pass
    
    def get_stats_from_class_mean(self):
        # between_var_inv = scipy.linalg.pinvh(self.between_var)
        # within_var_inv = scipy.linalg.pinvh(self.within_var)

        between_var_inv = inv(self.between_var)
        within_var_inv = inv(self.within_var)
        ...
        for i in range(self.stats.num_classes):
            ...
            if n != info.num_example:
                ...
                # mix_var = scipy.linalg.pinvh(mix_var)
                mix_var = inv(mix_var)
                ###
                # m_var_inv = scipy.linalg.pinvh(self.between_var + self.within_var / n)
                m_var_inv = inv(self.between_var + self.within_var / n)
                logdet_cov_x = np.linalg.slogdet(self.between_var)[1]
                logdet_cov_y = np.linalg.slogdet(self.within_var / n)[1]
                logdet_cov_m = np.linalg.slogdet(self.between_var + self.within_var / n)[1]
                logdet_mix_var = np.linalg.slogdet(mix_var)[1]
                ###
            ...
            # m_w = m - w
            # print('t')
            mw = mix_var.dot(between_var_inv.dot(m))
            stats_xxT = mix_var + w[:, None].dot(w[None, :])
            stats_yyT = mix_var + mw[:, None].dot(mw[None, :])
            ...
            # self.nllr_m += logdet_cov_m + np.trace(m[:, None].dot(m[None, :]).dot(m_var_inv))

            ###
            nllr_x_this = 0.5*self.dim*M_LOG_2PI + 0.5*logdet_cov_x + 0.5*np.trace(stats_xxT.dot(between_var_inv))
            self.nllr_x += nllr_x_this
            nllr_y_this = 0.5*self.dim*M_LOG_2PI + 0.5*logdet_cov_y + 0.5*np.trace(stats_yyT.dot(n * within_var_inv))
            self.nllr_y += nllr_y_this

            self.nllr_m += 0.5*self.dim*M_LOG_2PI + 0.5*logdet_cov_m + 0.5*np.trace(m[:, None].dot(m[None, :]).dot(m_var_inv))

            self.nllr_m_2 += nllr_x_this + nllr_y_this - 0.5*self.dim*M_LOG_2PI - 0.5*self.dim - 0.5*logdet_mix_var
            # self.nllr_m_2 += nllr_x_this + nllr_y_this - self.dim*M_LOG_2PI - self.dim - logdet_mix_var
            ###
        ...
        print('nllr_m:', self.nllr_m/self.stats.num_classes)
        print('nllr_m_2:', self.nllr_m_2/self.stats.num_classes)
        self.nllr_x = 0.0
        self.nllr_y = 0.0
        self.nllr_m = 0.0
        self.nllr_m_2 = 0.0
    ...
    def get_output(self):
        W_1 = inv(np.linalg.cholesky(self.within_var + 1e-15 * np.eye(self.dim)))
    ...
    def get_output_from_var(self, within_var, between_var, mean, dim):
        W_1 = inv(np.linalg.cholesky(within_var + 1e-15 * np.eye(dim)))
        between_var_proj = W_1.dot(between_var).dot(W_1.T)
    ...
    def get_stat(self):
        return [self.within_var, self.between_var]
